...
const Login = (props) => {
	...
	const { username, password, showPassword, error } = loginCredentials;
	...
	const loginFormFields = useMemo(
		() => getLoginFormFields(togglePassword),
		[togglePassword]
	);
				...
				<div className={styleElem.root}>
					<Card className={styleElem.card}>
						<div className={styleElem.logoWrapper}>
							<img className={styleElem.media} src={LOGO} title='SPI Global' />
						</div>
						<CardContent>
							<form autoComplete='off'>
								{Object.keys(loginFormFields).map((key) => {
									const showIconOne =
										key === 'password' && showPassword ? false : true;
									const type =
										key === 'password' && showPassword
											? 'text'
											: loginFormFields[key].type;
						...
						<CardActions className={styleElem.cardAction}>
							...
							>
								...
								{loading && (
									<CircularProgress size={14} className={styleElem.spinner} />
								)}
							</Button>
